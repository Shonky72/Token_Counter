...
def test_fetch_rate_limits_routes_to_base_url(monkeypatch):
    captured = {}

    def fake_request(url, headers, data=None):
        captured["url"] = url
        return True, "ok", {"x-ratelimit-limit-tokens": "5"}

    monkeypatch.setattr(probe, "_request", fake_request)
    probe.fetch_rate_limits("openai", "k", base_url="https://api.groq.com/openai/v1",
...
def test_validate_credential_populates_gauge(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(probe, "probe", lambda *a, **k: (True, "ok", {}))
    monkeypatch.setattr(
        probe, "fetch_rate_limits",
        lambda *a, **k: (True, "ok", _ANTHROPIC_HEADERS),
    )
    cfg = parse_config(
